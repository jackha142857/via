...
class Helix3Menu {
	...
	public function initMenu(){
		...
		foreach ($items as &$item) {
			...
			switch ($item->type) {
				// Flex Note: removed "continue" due to error in PHP 3. Changed to "break".
				case 'separator':
                break;
	...
	private function item($item, $extra_class=''){
		...
		if($icon = $item->params->get('icon')) {
			...
			//$fa5 = JHtml::_('select.option', $value, str_replace('fa-', '', $value) );
			//$arr[] = JHtml::_('select.option', $value, str_replace('fa-', '', $value) ); 
			//$fa4  = array('fa ');
			//$replace = array('');
			//$subject = 'A';
			//$fa5 = (str_replace($fa4, $replace, $subject));
			//$fa5 = str_replace("fa-", "", $icon);
